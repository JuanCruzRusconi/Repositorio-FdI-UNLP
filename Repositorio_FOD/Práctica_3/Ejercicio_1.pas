...
P1.4 Agregar al menú del programa del ejercicio 3, opciones para:
        ...
        if(opcion = 1) then begin
            ...
            if(opcion = 2) then begin
                ...
                // Parte D
                    ...
                    writeln("Ingrese el num de empleado a eliminar");
                    readln(empEliminar);

                    leer(empleados, e);

                    while(e.num <> valorAlto) and (e.num <> empEliminar) do
                        leer(empleados, e);

                    if(e.num = empEliminar) then begin
                        posEliminar:= filePos(empleados) - 1;

                        if(posEliminar <> fileSize(empleados)-1) then begin
                            seek(empleados, fileSize(empleados) - 1);
                            read(empleados, ultimo);

                            seek(empleados, posEliminar);
                            write(empleados, ultimo);
                        end;
                        seek(empleados, filSize(empleados) - 1);
                        truncate(empleados);
